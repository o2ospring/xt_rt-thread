...
void bsp_pwm_init(void)
{
	...
	//PWM1 工作参数设置-----------------------------------------------------------------------------------
	#ifdef XXX_PWM1_CLK_ENABLE
//	HAL_TIM_PWM_Stop(&XXX_TIMX_HANDLER, TIM_CHANNEL_1);//关闭PWM1输出(上电默认是关闭,平时必须关闭才能设置)
	...
	{}
	HAL_TIM_PWM_Start(&XXX_TIMX_HANDLER, TIM_CHANNEL_1); //启动PWM1输出
	#endif
	
	//PWM2 工作参数设置-----------------------------------------------------------------------------------
	#ifdef XXX_PWM2_CLK_ENABLE
//	HAL_TIM_PWM_Stop(&XXX_TIMX_HANDLER, TIM_CHANNEL_2);//关闭PWM2输出(上电默认是关闭,平时必须关闭才能设置)
	...
	{}
	HAL_TIM_PWM_Start(&XXX_TIMX_HANDLER, TIM_CHANNEL_2); //启动PWM2输出
	#endif
	
	//PWM3 工作参数设置-----------------------------------------------------------------------------------
	#ifdef XXX_PWM3_CLK_ENABLE
//	HAL_TIM_PWM_Stop(&XXX_TIMX_HANDLER, TIM_CHANNEL_3);//关闭PWM3输出(上电默认是关闭,平时必须关闭才能设置)
	...
	{}
	HAL_TIM_PWM_Start(&XXX_TIMX_HANDLER, TIM_CHANNEL_3); //启动PWM3输出
	#endif
	
	//PWM4 工作参数设置-----------------------------------------------------------------------------------
	#ifdef XXX_PWM4_CLK_ENABLE
//	HAL_TIM_PWM_Stop(&XXX_TIMX_HANDLER, TIM_CHANNEL_4);//关闭PWM4输出(上电默认是关闭,平时必须关闭才能设置)
	...
	{}
	HAL_TIM_PWM_Start(&XXX_TIMX_HANDLER, TIM_CHANNEL_4); //启动PWM4输出
	#endif
	...
	// 临时修改[50%占空比方波输出]例子!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	// XXX_TIMX->XXX_PWM1_CCRx = XXX_TIMX_PWM_FREVOL/2; -> 50%点空比方波输出
	// XXX_TIMX->XXX_PWM2_CCRx = XXX_TIMX_PWM_FREVOL/2; -> 50%点空比方波输出
	// XXX_TIMX->XXX_PWM3_CCRx = XXX_TIMX_PWM_FREVOL/2; -> 50%点空比方波输出
	// XXX_TIMX->XXX_PWM4_CCRx = XXX_TIMX_PWM_FREVOL/2; -> 50%点空比方波输出
